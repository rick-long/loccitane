...
public class BulkAPI {
	...
	private static final Logger logger = Logger.getLogger(BulkAPI.class);

	
	public static void main(String[] args)
      throws AsyncApiException, ConnectionException, IOException {
		BulkAPI example = new BulkAPI();
		...
		example.runSample("Contact", APIConstant.SF_USERNAME, APIConstant.SF_PASSWORD_TOKEN, "/Users/liusiping/Documents/data_from_sot_member_Contact_20170809122825.csv",null);
    ...
    public void runSample(String sobjectType, String userName,
              String password, String sampleFileName,String operationEnum)
            throws AsyncApiException, ConnectionException, IOException {
    	logger.debug("------BulkAPI-----runSample------userName:"+userName+",password:"+password+",operationEnum:"+operationEnum);
        BulkConnection connection = getBulkConnection(userName, password);
        JobInfo job = createJob(sobjectType, connection,operationEnum);
     ...
     * @throws AsyncApiException
     */
    private JobInfo createJob(String sobjectType, BulkConnection connection,String operationEnum)
          throws AsyncApiException {
        JobInfo job = new JobInfo();
        job.setObject(sobjectType);
        if(operationEnum.equals(APIConstant.OPERATION_ENUM_INSERT)){
        	job.setOperation(OperationEnum.insert);
        }else{
        	job.setOperation(OperationEnum.upsert);
        }
        if(sobjectType.equals(APIConstant.SF_OBJECT_ACCOUNT)){
        	job.setExternalIdFieldName("SOT_Account_Number__c");
        }else if(sobjectType.equals(APIConstant.SF_OBJECT_CONTACT)){
        	job.setExternalIdFieldName("SOT_Account_Number_2__c");
        }
    ...
    private BulkConnection getBulkConnection(String userName, String password)
        ...
        partnerConfig.setAuthEndpoint(APIConstant.SF_END_POINT);

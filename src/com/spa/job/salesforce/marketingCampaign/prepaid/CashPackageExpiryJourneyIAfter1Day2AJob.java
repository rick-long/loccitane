...
public class CashPackageExpiryJourneyIAfter1Day2AJob extends OpenSessionQuartzJobBean {
	
    private static final String marketing_campaign_code="7";
    private static final String marketing_campaign_subCode="cp2a";// no cash remaining
    ...
    protected void executeJob(JobExecutionContext ctx) throws JobExecutionException {
        String loginUser ="CashPackageExpiryJourneyIAfter1Day2AJob";
		...
		try {
			userIds = prepaidService.getMemberIdsByExpiryPrepaid(DateUtil.dateToString(fromDate, "yyyy-MM-dd"), DateUtil.dateToString(toCalendar.getTime(), "yyyy-MM-dd"), CommonConstant.PREPAID_TYPE_CASH_PACKAGE,false);
        ...
        if(prevUmps !=null && prevUmps.size()>0){
            System.out.println(" run ----CashPackageExpiryJourneyIAfter1Day2AJob---  prev id size ----"+prevUmps.size());
        ...
        if(postIds !=null && postIds.size()>0){
        	System.out.println(" run ----CashPackageExpiryJourneyIAfter1Day2AJob---  postIds id size ----"+postIds.size());
            User postUser =null;
        ...
        System.out.println(" run ----CashPackageExpiryJourneyIAfter1Day2AJob---end--");

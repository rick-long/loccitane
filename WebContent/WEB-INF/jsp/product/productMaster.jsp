...
<div class="row new-row-width" id="master-add">
	<!-- form: -->
    <section>
    	<div class="col-lg-6 col-lg-offset-0 form-group-margin">
            <form:form modelAttribute="productAddVO" id="masterForm" method="post" class="form-horizontal" action=''>
                 ...
                 <legend><spring:message code="label.product.details"/></legend>
                	<div class="form-group">
                        <label class="col-lg-3 control-label"><spring:message code="label.name"/></label>
                        <div class="col-lg-5">
                           <form:input path="name" id="name" class="form-control"/>
                        </div>
                    </div>
                   	<div class="form-group">
						<label class="col-lg-3 control-label"><spring:message code="label.brand"/></label>
					    <div class="col-lg-5">
                    ...
                    <div class="form-group">
						<label class="col-lg-3 control-label"><spring:message code="label.parent.category"/></label>
					    <div class="col-lg-5">
				   ...
				   <div class="form-group">
						<label class="col-lg-3 control-label"><spring:message code="label.sub.category"/></label>
					    <div class="col-lg-5">
...
$(document).ready(function() {
  ...
  //category casecade select
	...
	categorysCasecade($('#parentCategoryId',masterDiv),$('#secondCategoryId',masterDiv),pCategoryChangeUrl);
});
...
$($('#submitBtn',masterDiv)).click(function() {
	    	 ...
	    	 $.each(json.errorFields, function (index, item) {
	    		 ...
	    		 if(fname =='success'){
	    			 ...
	    			 $('#myTabs a').each(function(e){
	    		         ...
	    		         if(tabname =='po'){
	    				 	var url='<c:url value="/product/toAddPo?productId="/>'+item.errorMessage;
	    				 	$(this).attr("data-url",url);
	    				 	
	    				 	//this.preventDefault();//阻止a链接的跳转行为
   			                $.ajax({
   			                   url: url,
   			                   type: "POST",
   			                   dataType: "text",
	...
	function pdCasecade(casecadeParameter,casecadeDomId,initVal){
		$.ajax({
			 url:'<c:url value="/product/listPd"/>',
			 type: "post",
			 dataType: "text",
			 data: {secondCategoryId:casecadeParameter},
			 success: function(data) {

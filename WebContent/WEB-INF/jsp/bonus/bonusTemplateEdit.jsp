...
<div>
    <form:form method="post" modelAttribute="bonusTemplateVO" class="form-horizontal" enctype="multipart/form-data">
        <form:hidden path="id"/>
        <div class="form-group">
            <label for="name" class="col-lg-4 control-label"><spring:message code="label.name"/></label>
            <div class="col-lg-5">
                <input type="text" id="name" name="name" value="${bonusTemplate.name}" class="form-control">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-lg-5">
                <textarea id="description" name="description" class="form-control">${bonusTemplate.description}</textarea>
            </div>
        </div>
        <div class="form-group">
            <label for="content" class="col-lg-4 control-label"><spring:message code="label.content"/></label>
            <div class="col-lg-5">
                <textarea id="content" name="content" class="form-control">${bonusTemplate.content}</textarea>
            </div>
        </div>
        <div class="row new-row-width">
             ...
             <table class="table table-striped">
                 ...
                 <tbody id="body">
                 <c:forEach items="${bonusTemplate.bonusAttributeKeys}" var="item" varStatus="status">
                     <tr>
                         <td>
                         ...
                         <td>
                             <button type="button" class='btn btn-warning removePOBtn<c:if test="${status.first}"> hide</c:if>'>Remove</button>
                         </td>
                     </tr>
                 </c:forEach>
                 </tbody>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        submitBtn.click(function () {
             ...
             form.find('input[name^="bonusAttributeKeyVO"]').remove();
             // 添加参数
             var html = [];
             var trs = $('#body', Dialog.getContext()).find("tr");
             trs.each(function (index, item) {
                 var $item = $(item);
                 html.push(String.format('<input type="hidden" name="bonusAttributeKeyVO[{0}].reference" value="{1}"/>', index, $item.find('.reference').val()));
                 html.push(String.format('<input type="hidden" name="bonusAttributeKeyVO[{0}].name" value="{1}"/>', index, $item.find('.name').val()));
                 html.push(String.format('<input type="hidden" name="bonusAttributeKeyVO[{0}].description" value="{1}"/>', index, $item.find('.description').val()));
                 html.push(String.format('<input type="hidden" name="bonusAttributeKeyVO[{0}].displayOrder" value="{1}"/>', index, index + 1));
             });
            ...
            var formData = new FormData(form[0]);
            $.ajax({
                url: '<c:url value="/bonus/bonusTemplateSave"/>',
                type: 'POST',
                data: formData,
                async: false,

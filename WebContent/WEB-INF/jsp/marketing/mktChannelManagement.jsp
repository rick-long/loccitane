...
<div class="management" id="userlevel-search">
    <h3><spring:message code="label.mkt.channel.search.management"/>
        <a data-permission="marketing:toMktChannelAdd" href='<c:url value="/marketing/toMktChannelAdd"/>' class="btn btn-primary dialog" data-draggable="true"
           data-title='<spring:message code="label.mkt.channel.add.management"/>'>
            <i class="glyphicon glyphicon-plus"></i><spring:message code="label.button.add"/>
        </a>
    </h3>
    <c:url var="url" value="/marketing/mktChannelList"/>
    <form:form modelAttribute="mktChannelListVO" id="commonSearchForm" method="post" class="form-inline" action="${url}">

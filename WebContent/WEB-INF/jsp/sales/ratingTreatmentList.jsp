...
<div class="home-shop">
	<div class="left grade">
		<div class="top">
			...
			<span><spring:message code="label.sales.satisfactionlevelavg"/> </span>
		</div>
		<canvas id="canvas" width="210" height="210" data-bg="#FFA829" data-score="${satisfactionLevelStarAvg}"></canvas>
	...
	<div class="right grade">
		<div class="top">
			...
			<span><spring:message code="label.sales.valueformoneyavg"/> </span>
		</div>
		<canvas id="canvas2" width="210" height="210" data-bg="#29CEFF" data-score="${valueForMoneyStarAvg}"></canvas>
		<p>Bass on <c:if test="${empty reviewSum }">0</c:if>
			<c:if test="${not empty  reviewSum }">${reviewSum}</c:if> reviews</p>
	</div>
</div>
...
<div class="home-shop rat-content">
	<ul>
		<li class="rat-title">
			<h1><spring:message code="label.sales.treatmentname"/> </h1>
			<p class="rat-cus"><spring:message code="label.sales.satisfactionlevelrating"/> </p>
			<p><spring:message code="label.sales.valueformoneyrating"/> </p>
		</li>
		<c:forEach items="${top10List}" var="reviewRatingTreatment">
		<li>
			<a href="/?loadingUrl=/review/ratingTreatmentDetails?productId=${reviewRatingTreatment.productId}">
				<span class="type-name">${reviewRatingTreatment.productName}</span>
				<span>${reviewRatingTreatment.satisfactionLevelAvg}</span>
				<p class="star" data-bg="yellow" data-score="${reviewRatingTreatment.satisfactionLevelAvg}"></p>
				<span>${reviewRatingTreatment.valueForMoneyAvg}</span>
				<p class="star" data-bg="blue" data-score="${reviewRatingTreatment.valueForMoneyAvg}"></p></a>
		</li>
		</c:forEach>
	</ul>
</div>

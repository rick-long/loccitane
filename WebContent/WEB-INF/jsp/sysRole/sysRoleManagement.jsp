...
<div class="management" id="userlevel-search">
    <h3><spring:message code="label.sys.role.search.management"/>
        <a data-permission="sysRole:sysRoleAdd" href='<c:url value="/sysRole/sysRoleAdd"/>' class="btn btn-primary dialog" data-draggable="true"
           data-title='<spring:message code="label.sys.role.add.management"/>'>
            <i class="glyphicon glyphicon-plus"></i><spring:message code="label.button.add"/>
        </a>
    </h3>
    <c:url var="url" value="/sysRole/list"/>
    <form:form modelAttribute="sysRoleListVO" id="commonSearchForm" method="post" class="form-inline" action="${url}">
   <div class="row">
      <div class="col-sm-12">

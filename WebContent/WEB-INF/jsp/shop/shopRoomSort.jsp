...
<c:url var="url" value="/shop/sortRoom"/>
    ...
    <table class="table table-striped table-hover" <%--style="align:center"--%>>
        ...
        <c:forEach items="${roomList}" var="rl">
            ...
            <tr>
                <td>
                        ${rl.name}
                    <input type="hidden" value="${rl.id}" id="id" name="roomSortVOList[${listSum}].id"/>
                </td>
                    <td><input id="sort" class="form-control sort" name="roomSortVOList[${listSum}].sort" size="3" value="${sum}" style="width: 60px;display: none"/></td>
                <td>
                ...
                <td>
                    <a type="button" class="btn btn-info" name="downMove"
                       value='<spring:message code="label.button.move.down"/>'>

...
(function (factory) {
    ...
    Item.prototype = {
        init: function () {
            ...
            this.$element.on('click', $.proxy(this, 'click')); // 注册选择事件
        },
        close: function () {
        ...
        click: function (event) {
            // 处理选择事件
            // 叶子节点点击处理
            var selectCategoryContext = this.$element.parents('.select-category');
            var selectable = selectCategoryContext.data('selectable');
            if (this.$element.hasClass('category') && selectable.indexOf('category') == -1) {
                event.stopPropagation();
            } else if (this.$element.hasClass('product') && selectable.indexOf('product') == -1) {
                event.stopPropagation();
            } else if (this.$element.hasClass('productOption') && selectable.indexOf('option') == -1) {
                event.stopPropagation();
            }
        }
    };
